...
-   I would remove the outliers, because while a transformation to a logarithmic transformation normalizes the observations, it causes the visualization to lose much of its ability to be interpreted. Also, there are only 2 observations which are outliers.
...
-   This line plot is no longer appropriate because both females and males named "Chandler" are counted in the yearly count. This changes the trends in name usage compared to restricting the other names by the sex.
...
stocks_long %>%
  ggplot(aes(x = start_date, 
             ...
             color = Stock_Type)) +
  geom_line() +
  geom_area(aes(fill = Stock_Type), position = "identity", alpha = 0.3) +
  theme_minimal() 
  
```

-   A line plot would be better because it emphasizes the trend of price over time, while the area plot emphasizes the volume of price taken up by each stock.
...
# instead use geom_hex() to de-clutter
# also give you an approximate sample size
# also gives the distribution in a better way
# lose the ability to use color for another variable in the plot

# most diamonds are low carat and low price (but can't us color as an extra aesthetic now)
```

```{r}
# REALLY Big sample size:
library(tidyverse)
n <- 10000000
x <- rnorm(n, 0, 1)
y <- x * rgamma(n, 3, 2)
...
## if there are outliers or any data entry errors, there is a chance they are not plotted in the sample you selected
```
...
```{r}
library(GGally)
ggpairs(data = mlb_small, columns = 6:11, progress = F)
...
-   From the correlation plot there does not appear to be any association between x1 and x2.
...
-   There appears to be no clear association between x1 and but x2 and y seems to have a quadratic relationship.
...
-   Correlation coefficients only work if relationship is linear, so it is not correct to solely use the correlation plot to determine if there are relationships between variable and responses.

Multicolinearity:
-   a statistical phenomenon that occurs when two or more independent variables in a regression model are highly correlated with each other. Can also cause cause coefficients in a model to go wacky.
